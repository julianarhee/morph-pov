...
%         end
        ...
        curr_mfile_idx = ~cellfun('isempty', strfind(mfiles, sprintf('%s_compare_pixels', stimset)))
...
%         
        ...
        if isempty(curr_mfile)
            ...
            curr_mfile = sprintf('%s_compare_pixels.mat', stimset);
            save_new = 1;
        else
        ...
        if isfield(M, 'pdist') && isfield(M.pdist, corrType)
            ...
            for idx=1:length(imnames)
                ...
                sampled_image_vects = [sampled_image_vects; reshape(curr_im, 1, numel(curr_im))];  % F = [F curr_feat.featureVector']; doesn't work.. too big
            end
            nsamples = length(imnames);

            dist_mat = pdist(sampled_image_vects, corrType);
