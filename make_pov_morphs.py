...
def custom_mapping(transform, transform_idx, curr_diff_vec, n_total_morphs,
                   map_type='linear', constrain_start=0, constrain_end=-1, a=1, b=0):
    ...
    '''
    if map_type == 'linear':
        custom_steps = linmap(curr_diff_vec[transform_idx], n_total_morphs)
    elif map_type == 'sigmoid':
        custom_steps = sigmap(curr_diff_vec[transform_idx], n_total_morphs,
                                                        constrain=[int(constain_start), int(constrain_max)],
                                                        a=a, b=b)
    return custom_steps

def generate_custom_steps(p_steps, curr_idx, curr_diff_vec, n_total_morphs, transform_dict):
    '''
    # BASE, i.e., sfera (culo) ----------------------------
    ...
    if custom_sphere1 == True:
        ...
        transform_dict = {'translation': {'index': 0,
                                          'map_type': 'linear',
                                          'constrain': None,
                                          'a': None,
                                          'b': None},
                          ...
                          'rotation': {'index': 2,
                                       ...
                                       'a': None,
                                       'b': None} }
        p_steps = generate_custom_steps(p_steps, curr_idx, curr_diff_vec, n_total_morphs)
    '''

    for transform in transform_dict.keys():

        p_steps[curr_idx].__dict__[transform] = custom_mapping(transform, transform_dict[transform]['index'],
                                                            curr_diff_vec, n_total_morphs,
                                                            map_type=transform_dict[transform]['map_type'],
                                                            constrain=transform_dict[transform]['constrain'],
                                                            a=transform_dict[transform]['a'],
                                                            b=transform_dict[transform]['b'])
    return p_steps
...
#def run():
    ...
    for morph_num in range(n_total_morphs):
		...
		{k+str(i+1): '<%s>' % ', '.join(map(str, p_start[i].__dict__[k] + p_steps[i].__dict__[k][morph_num]))
                                              for i in range(len(p_start)) for k in p_start[i].__dict__.keys()}
